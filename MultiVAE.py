...
if __name__ == '__main__':
    ...
    iterations = [i for i in range(fl.metrics['card'])]
    training_step = [i for i in np.arange(len(fl.metrics['test_hit']))]

    axs[0].set_title('Diversity of Items')
    axs[0].plot(iterations, fl.metrics['card'], color = 'blue', linestyle = 'dashed')
    ...
    axs[2].plot(iterations, fl.metrics['S_ind'], color = 'blue', linestyle = 'dashed')
    axs[2].set_xticks(range(len(iterations)))
    axs[2].vlines(np.arange(len(iterations), step=m), ymin=min(fl.metrics['S_ind']), ymax= max(fl.metrics['S_ind']), colors='red',linestyles='dotted')
    # Adjust layout
    plt.tight_layout()
    # Show plots
